...
A web chat where students in a specific department/batch/group can anonymously share
...
- **Owner** — holds the root secret. Everything a co-admin can do, plus managing
  departments/batches/groups and creating/revoking co-admins.
...
Room identity is the triple department → batch → group. `groups` *is* the room.
...
### Room selection
`/` presents Department → Batch → Group. The last-used room is remembered in
`localStorage` and offered as a one-click resume. Room URL is
`/c/[dept-slug]/[batch-number]/[group-label]`.
...
Owner-only pages:
- `/sudo/structure` — create, rename, delete departments, batches, groups. Deleting
  cascades and is confirmed with a typed room name.
- `/sudo/admins` — create a co-admin (**secret is generated by the server and shown
  exactly once**, never retrievable), revoke a co-admin, see last-active.
