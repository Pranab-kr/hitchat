...
export default function StructureLoading() {
  return (
    <main className="mx-auto max-w-2xl px-6 py-12" aria-busy="true" aria-label="Loading structure">
      <Block className="h-4 w-12" />
      <Block className="mt-4 h-9 w-36" />
      <Block className="mt-2 h-5 w-72 max-w-full" />
      <div className="mt-8 space-y-6">
        {[0, 1, 2].map((item) => (
          <section key={item} className="rounded-card border border-hairline bg-surface px-5 py-5">
            ...
            <Block className="mt-3 h-10 w-28" />
          </section>
        ))}
      </div>
    </main>
